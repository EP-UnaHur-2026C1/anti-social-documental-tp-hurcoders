const { Post, User, PostImage, Comment, Tag, PostTag } = require('../models');
...
const buildPosts = async (posts) =>{
  ...
  const [users, images, postTags, comments] = await Promise.all([
    //$in es un operador de consulta de MongoDB. Es el equivalente al IN de SQL
    User.find({ _id: { $in: userIds } }),
    PostImage.find({ postId: { $in: postIds } }),
  ...
  //Emsamblaje de los datos
    ...
    ...post.toObject(),
    user: users.find((u) => u._id.equals(post.userId)) || null,
    images: images.filter((img) => img.postId.equals(post._id)),
...
const addImagePost = async (req, res) => {
  ...
  const image = await PostImage.create({ imageUrl: req.body.imageUrl, postId: req.params.id });
...
const uploadImagePost = async (req, res) => {
  ...
  const image = await PostImage.create({ imageUrl, postId: req.params.id });
  res.status(201).json(image);
};

const removeImagePost = async (req, res) => {
  const image = await PostImage.findOneAndDelete({
